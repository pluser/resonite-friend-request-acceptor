...
export class ResoniteClient extends EventEmitter {
  ...
  async start(): Promise<void> {
    ...
    const noopEvents = [
      ...
      "RemoveSession",
      "ReceiveStatusUpdate",
      // Undocumented event observed in production
      "debug",
    ];
   ...
   * Returns `null` on failure so callers can distinguish errors from empty lists.
   */
  async getContacts(): Promise<ResoniteContact[] | null> {
